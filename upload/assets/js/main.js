...
// lang btn 
// Disable language button globally
document.addEventListener("DOMContentLoaded", () => {
    const langBtn = document.getElementById("langBtn");
    if (!langBtn) return; // if not found, skip

    // Remove any existing click events (if already set in HTML)
    const newBtn = langBtn.cloneNode(true);
    langBtn.parentNode.replaceChild(newBtn, langBtn);

    // Add a disabled click event
    newBtn.addEventListener("click", (e) => {
        e.preventDefault();
        // Disabled for now — does nothing
        console.log("Language switch is disabled for now.");
    });
});
    

document.getElementById("langBtn").addEventListener("click", function () {
    const currentPage = window.location.pathname.split("/").pop(); // page.html
    window.location.href = "/rtl/" + currentPage; 
});